...
async function testCompleteFlow() {
  ...
  try {
    ...
    console.log(
      `   Nombre: ${validateResponse.data.invitacion.nombre_completo}`
    );
    console.log(`   Rol: ${validateResponse.data.invitacion.rol}`);
    console.log(
      `   Expira: ${new Date(
        validateResponse.data.invitacion.fecha_expiracion
      ).toLocaleString()}`
    );
    ...
    console.log(
      `   Token: ${newUserLogin.data.data.accessToken.substring(0, 20)}...`
    );
    ...
    try {
      ...
      if (
        ...
        err.response?.data?.error?.includes('utilizada')
      ) {
    ...
    const listResponse = await axios.get(`${API_URL}/invitaciones`, {
      headers: { Authorization: `Bearer ${adminToken}` },
    });
    ...
    console.log(
      `   Pendientes: ${listResponse.data.data.length - usedInvitations.length}`
    );
