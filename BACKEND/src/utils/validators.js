...
const createUsuarioValidators = [
  ...
  body('rol')
    .optional()
    .isIn(['administrador', 'usuario_consulta']).withMessage('Rol inválido')
];
...
const createPersonaValidators = [
  ...
  body('dni')
    .optional()
    .trim()
    .isLength({ max: 20 }).withMessage('El DNI no puede exceder 20 caracteres'),
  body('email')
    .optional()
    .trim()
...
const createRegistroValidators = [
  ...
  body('ufi')
    .optional()
    .trim()
    .isLength({ max: 100 }).withMessage('UFI no puede exceder 100 caracteres'),
  body('numero_legajo')
    .optional()
    .trim()
    .isLength({ max: 100 }).withMessage('Número de legajo no puede exceder 100 caracteres'),
  body('estado_causa')
    .optional()
    .isIn(['abierta', 'cerrada', 'en_proceso']).withMessage('Estado de causa inválido')
];
...
const searchValidators = [
  query('termino')
    .optional()
    .trim()
    .isLength({ min: 1, max: 200 }).withMessage('Término de búsqueda debe tener entre 1 y 200 caracteres'),
  query('page')
    .optional()
